...
module UserBehavior
  ...
  def self.generate_user_behavior(user_hash ,references=nil, search_conditions={})
    terms = $db_rightful_name=="egypt" ? @egypt_keys : @tunisia_keys
    tweets = BehaviorTweet.all(search_conditions.merge(:screen_name => user_hash[:screen_name]), :order => "created_at")
