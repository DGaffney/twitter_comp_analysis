...
class NewDeGilader
  ...
  def gilad_clean
    # DataMapper.repository(database) do
      ...
      all_tweet_ids = DataMapper.repository(:default).adapter.select("SELECT tweets.id FROM tweets inner join users where (tweets.in_reply_to_status_id=0 and tweets.text like 'rt:%') order by rand() limit 100") # or (tweets.screen_name=users.screen_name and users.followers_count=0)  << This will pull out users with zero follower counts
